...
export const App: React.FC = () => {
  ...
  return <div className={`min-h-screen font-sans transition-colors duration-200 ${darkMode ? 'dark bg-[#0b1120] text-slate-100' : 'bg-[#f6f8fb] text-slate-900'}`}>
    ...
    <main className="mx-auto w-full max-w-[1440px] px-4 py-6 sm:px-6 lg:px-8 lg:py-8">
      {!showWorkspace && !demoLoading && !loadError && <section className="animate-in flex min-h-[calc(100vh-180px)] items-center justify-center py-8">
        <div className="w-full max-w-5xl">
          <div className="mx-auto max-w-3xl text-center">
            <div className="mx-auto mb-6 flex h-16 w-16 items-center justify-center rounded-2xl bg-slate-950 text-white shadow-xl dark:bg-white dark:text-slate-950"><Logo size={42} compact /></div>
            <div className="mb-4 inline-flex items-center gap-2 rounded-full border border-blue-200 bg-blue-50 px-3 py-1.5 text-[11px] font-bold uppercase tracking-[0.16em] text-blue-700 dark:border-blue-900 dark:bg-blue-950/40 dark:text-blue-300"><Sparkles className="h-3.5 w-3.5" /> Interactive demo engagement</div>
            <h1 className="text-4xl font-black tracking-tight text-slate-950 dark:text-white sm:text-5xl">From messy business data to a decision-ready answer.</h1>
            <p className="mx-auto mt-5 max-w-2xl text-base leading-7 text-slate-500 dark:text-slate-400 sm:text-lg">See Consulting in a Box run a realistic NovaMart profitability case — from data and KPIs to driver diagnosis, recommendations, scenarios, and an executive report.</p>
            <button onClick={runDemo} className="mt-8 inline-flex items-center gap-2 rounded-xl bg-blue-600 px-6 py-3.5 text-sm font-bold text-white shadow-lg shadow-blue-600/20 transition hover:bg-blue-700">Run Demo Engagement <ArrowRight className="h-4 w-4" /></button>
          </div>

          <div className="mx-auto mt-14 grid max-w-4xl gap-3 sm:grid-cols-4">
            {[{ icon: Database, title: 'Ingest', text: 'Structured business data' }, { icon: BarChart3, title: 'Analyze', text: 'Deterministic KPI engine' }, { icon: TrendingUp, title: 'Diagnose', text: 'Drivers & evidence' }, { icon: FileText, title: 'Decide', text: 'Actions & scenarios' }].map(({ icon: Icon, title, text }) => <div key={title} className="rounded-2xl border border-slate-200 bg-white/80 p-4 text-center shadow-sm dark:border-slate-800 dark:bg-slate-900/70"><Icon className="mx-auto h-5 w-5 text-blue-600" /><div className="mt-2 text-xs font-bold text-slate-800 dark:text-slate-100">{title}</div><div className="mt-1 text-[11px] text-slate-400">{text}</div></div>)}
          </div>

          <div className="mx-auto mt-5 flex max-w-4xl items-center justify-center gap-2 text-[11px] text-slate-400"><ShieldCheck className="h-3.5 w-3.5" /> Deterministic calculations · Evidence-backed insights · INR reporting</div>
        </div>
      </section>}
