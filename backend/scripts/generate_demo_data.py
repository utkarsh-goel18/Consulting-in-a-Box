...
def generate_datasets(output_dir: str, scale: str = "demo"):
    ...
    print(f"Generating synthetic NovaMart data (scale: {scale}) in {output_dir}...")

    # Determine counts based on scale
    if scale == "full":
        n_customers = 100000
        n_orders = 500000
    else:
    ...
    print(f"[OK] Products generated: {len(products_df)} rows")
    ...
    for i in range(1, n_customers + 1):
        ...
        # Anomaly 3: Elevated churn in Tier 2 regions
    ...
    print(f"[OK] Customers generated: {len(customers_df)} rows")

    # 3. ORDERS & ORDER ITEMS
    # Date spans across Q2 2024 (April-June) and Q3 2024 (July-Sept)
    ...
    for ord_num in range(1, n_orders + 1):
        ...
        # Anomaly 1: Delivery costs surge in Q3, specifically FastLogistics (+17.1%)
        ...
        # Anomaly 2: AOV shrinkage in Q3 (fewer items and higher discounts)
        if is_q3:
        ...
        for _, prod_row in selected_prods.iterrows():
            ...
            # Anomaly 5: Slight margin erosion on Electronics in Q3
    ...
    print(f"[OK] Orders generated: {len(orders_df)} rows, Order Items: {len(order_items_df)} rows")

    # 4. MARKETING SPEND
    # Anomaly 4: Paid Social CAC increases sharply in Q3 with diminishing conversions
    marketing_records = []
    spend_id = 1
    cur_d = q2_start
    while cur_d <= q3_end:
    ...
    print(f"[OK] Marketing Spend generated: {len(marketing_df)} rows")
    ...
    print(f"[OK] Expenses generated: {len(expenses_df)} rows")
    ...
    print(f"[OK] Returns generated: {len(returns_df)} rows")
    print(f"Dataset generation complete for NovaMart in: {output_dir}\n")

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Generate synthetic business datasets for Consulting in a Box.")
    parser.add_argument("--output", type=str, default="backend/data", help="Output directory path")
    parser.add_argument("--scale", type=str, choices=["demo", "full"], default="demo", help="Dataset size scale")
    args = parser.parse_args()
    generate_datasets(args.output, args.scale)
