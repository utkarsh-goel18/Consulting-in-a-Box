...
from app.api.demo import clear_dashboard_cache
...
async def upload_dataset(
    ...
    workspace_name: str = Form("Uploaded Workspace"),
):
    """Ingest a dataset into the active analytical workspace and invalidate all derived state."""
    ...
    repo.workspace_name = workspace_name.strip() or "Uploaded Workspace"
    repo.is_demo_workspace = False
    invalidate()
...
def get_table_preview(name: str):
    ...
    return {"dataset_name": name, "total_rows": len(df), "columns": list(df.columns), "records": df.head(100).fillna("").to_dict(orient="records")}
