...
def generate_consulting_pdf(dashboard_data: Dict[str, Any], output_path: str = None) -> bytes:
    """Generate the data-driven ten-section executive report shown in the app."""
    buffer = io.BytesIO()
    ...
    eyebrow = ParagraphStyle("eyebrow", parent=small, fontName="Helvetica-Bold", textColor=colors.HexColor("#2563eb"))
    story += [
        Paragraph("CONSULTING IN A BOX", eyebrow),
        Paragraph("Executive Diagnostic & Decision Intelligence Report", title),
        Paragraph(f"{company} · {period} · INR", body),
        HRFlowable(width="100%", thickness=1, color=colors.HexColor("#0f172a")),
        Spacer(1, 7*mm),
    ]
    story.append(Paragraph("1. Executive Summary", h1))
    story.append(Paragraph(f"Net operating profit moved from {format_inr(k.get('net_profit_prior', 0))} to {format_inr(k.get('net_profit_current', 0))} ({k.get('net_profit_growth_pct', 0):+.1f}%) while revenue changed {k.get('revenue_growth_pct', 0):+.1f}%. The driver tree, P&L bridge and evidence vault are generated from the same deterministic analytical snapshot.", callout))
    ...
    for label, p, c, change in [("Revenue", k.get("revenue_prior", 0), k.get("revenue_current", 0), k.get("revenue_growth_pct", 0)), ("Gross Profit", k.get("gross_profit_prior", 0), k.get("gross_profit_current", 0), k.get("gross_profit_growth_pct", 0)), ("Net Profit", k.get("net_profit_prior", 0), k.get("net_profit_current", 0), k.get("net_profit_growth_pct", 0)), ("AOV", k.get("aov_prior", 0), k.get("aov_current", 0), k.get("aov_growth_pct", 0)), ("CAC", k.get("cac_prior", 0), k.get("cac_current", 0), k.get("cac_growth_pct", 0))]:
    ...
    for insight in dashboard_data.get("insights", []): story.append(Paragraph(f"<b>[{insight.get('classification')}] {insight.get('headline')}</b> — {insight.get('narrative')}", body))
    story.append(Paragraph("5. Root Cause Driver Tree", h1))
    root = dashboard_data.get("driver_tree", {})
    story.append(Paragraph(f"<b>{root.get('label','Net Profit')}</b>: {format_inr(root.get('prior_value',0))} → {format_inr(root.get('current_value',0))} ({root.get('delta_pct',0):+.1f}%). Child drivers are calculated from the same P&L bridge and ranked by observed impact.", body))
    for child in root.get("children", []):
        ...
        for leaf in child.get("children", []): story.append(Paragraph(f"&nbsp;&nbsp;&nbsp;– {leaf.get('label')}: {leaf.get('delta_pct',0):+.1f}%", small))
    story.append(Paragraph("6. Supporting Evidence & Methodology", h1))
    story.append(Paragraph("Evidence IDs attached to major findings map to executable analytical methodology, SQL aggregation and aggregate outputs in the platform evidence vault. FACT, INSIGHT and HYPOTHESIS are deliberately separated so causal claims are not presented as measured truth.", body))
