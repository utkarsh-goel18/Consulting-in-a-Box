...
def generate_fast_demo_datasets(output_dir: str, n_customers: int = 100_000, n_orders: int = 500_000) -> None:
    """Generate a deterministic, internally reconciled NovaMart fixture.

    The fixture is intentionally large enough to demonstrate enterprise-style
    profiling (100k customers / 500k orders) while using vectorized operations.
    Q3 is engineered through records—not hard-coded dashboard narratives—to
    exhibit lower order volume/AOV, carrier inflation, paid-social decay,
    higher returns and higher churn.
    """
    ...
    churn_q2 = rng.random(n_customers) < 0.080
    churn_q3 = rng.random(n_customers) < 0.112
    ...
    partner = rng.choice(partners, n_orders, p=[0.45, 0.33, 0.22])
    ...
    delivery_cost = np.maximum(partner_base * np.where(q3_mask, partner_q3_multiplier, 1.0) + rng.normal(0, 5, n_orders), 80).round(2)

    # Two line items per order; order revenue and COGS are reconciled to these items.
    ...
    discount_rate = np.where(q3_mask, rng.uniform(0.095, 0.145, n_orders), rng.uniform(0.055, 0.090, n_orders))
    ...
    for day in day_index:
        ...
        for channel, monthly_base in channel_base.items():
            spend = monthly_base / 30.0 * rng.uniform(0.94, 1.06)
            if q3 and channel == "Paid Social":
                spend *= 1.12
            effective_cac = channel_cac[channel] * (1.38 if q3 and channel == "Paid Social" else 1.0)
            attributed_orders = max(1, int(spend / effective_cac))
            roas = roas_base[channel] * (0.74 if q3 and channel == "Paid Social" else 1.0)
            marketing_rows.append({
                "spend_id": f"MKT-{mkt_id:06d}", "spend_date": day.strftime("%Y-%m-%d"), "channel": channel,
                "campaign": "NovaMart Performance", "spend_amount": round(spend, 2),
                "impressions": int(spend * 70), "clicks": int(spend * 2.8),
                "attributed_orders": attributed_orders, "attributed_revenue": round(spend * roas, 2),
            })
            mkt_id += 1
        for category, monthly_base in [("Warehousing & Fulfillment", 105_000), ("Technology & Hosting", 66_000), ("General & Admin", 54_000)]:
            ...
            if q3 and category == "Warehousing & Fulfillment":
                amount *= 1.10
            expense_rows.append({
                "expense_id": f"EXP-{exp_id:06d}", "expense_date": day.strftime("%Y-%m-%d"),
                "category": category, "department": category.split(" & ")[0],
                "description": "NovaMart operating expense", "amount": round(amount, 2),
            })
            exp_id += 1
    ...
    if not returned.empty:
        ...
        return_dates = return_dates.clip(upper=pd.Timestamp("2024-09-30"))
    else:
        return_dates = pd.Series(dtype="datetime64[ns]")
