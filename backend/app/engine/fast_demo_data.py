...
DEMO_VERSION = "novamart-v4-coherent-inr"
...
def generate_fast_demo_datasets(output_dir: str, n_customers: int = 100_000, n_orders: int = 500_000) -> None:
    ...
    products = pd.DataFrame({"product_id": product_ids, "sku": [f"SKU-{i:04d}" for i in range(1, n_products + 1)], "product_name": [f"NovaMart Product {i}" for i in range(1, n_products + 1)], "category": np.tile(categories, n_products // len(categories)), "subcategory": [f"Subcategory {i % 10 + 1}" for i in range(n_products)], "unit_cost": unit_cost, "unit_price": unit_price, "supplier_name": [f"Supplier {i % 12 + 1}" for i in range(n_products)]})
    ...
    customers = pd.DataFrame({"customer_id": [f"CUST-{i:06d}" for i in customer_ids], "signup_date": pd.Timestamp("2023-01-01") + pd.to_timedelta(rng.integers(0, 610, n_customers), unit="D"), "customer_name": [f"Customer {i}" for i in customer_ids], "email": [f"client_{i}@example.com" for i in customer_ids], "region": rng.choice(regions, n_customers, p=[.23, .21, .22, .19, .15]), "tier": rng.choice(["Tier 1", "Tier 2"], n_customers, p=[.61, .39]), "customer_segment": rng.choice(segments, n_customers, p=[.70, .22, .08]), "acquisition_channel": rng.choice(channels, n_customers, p=[.28, .23, .20, .17, .12]), "churn_q2_status": rng.random(n_customers) < .080, "churn_q3_status": rng.random(n_customers) < .112})
    customers["churn_status"] = customers["churn_q3_status"]
    ...
    partner_base = np.select([partner == "FastLogistics", partner == "ExpressCargo"], [180., 165.], default=205.)
    partner_q3_multiplier = np.select([partner == "FastLogistics", partner == "ExpressCargo"], [1.137, 1.055], default=1.028)
    delivery_cost = np.maximum(partner_base * np.where(q3_mask, partner_q3_multiplier, 1.) + rng.normal(0, 5, n_orders), 80).round(2)
    ...
    items = pd.DataFrame({"order_item_id": [f"ITEM-{i:08d}" for i in range(1, item_count + 1)], "order_id": [f"ORD-{i + 1:07d}" for i in item_order_index], "product_id": products.iloc[item_product_index]["product_id"].to_numpy(), "quantity": qty, "unit_price": item_prices, "unit_cogs": item_cogs, "item_total": item_total.round(2)})
    ...
    orders = pd.DataFrame({"order_id": [f"ORD-{i:07d}" for i in range(1, n_orders + 1)], "customer_id": customers.iloc[customer_idx]["customer_id"].to_numpy(), "order_date": pd.to_datetime(dates).strftime("%Y-%m-%d"), "order_status": status, "payment_method": rng.choice(["Credit Card", "UPI", "Net Banking", "COD"], n_orders, p=[.37, .38, .12, .13]), "shipping_partner": partner, "gross_amount": gross_by_order.round(2), "discount_amount": discount_amount.round(2), "net_amount": net_amount, "delivery_fee": np.where(net_amount > 2200, 0., 85.), "delivery_cost": delivery_cost})
    ...
    for day in day_index:
        ...
        for channel, monthly_base in channel_base.items():
            ...
            if q3 and channel == "Paid Social": spend *= 1.12
        ...
        for category, monthly_base in [("Warehousing & Fulfillment", 105_000), ("Technology & Hosting", 66_000), ("General & Admin", 54_000)]:
            ...
            if q3 and category == "Warehousing & Fulfillment": amount *= 1.10
    ...
    return_dates = pd.to_datetime(returned["order_date"]).to_numpy() + rng.integers(3, 11, len(returned)).astype("timedelta64[D]") if not returned.empty else np.array([], dtype="datetime64[ns]")
    if len(return_dates): return_dates = np.minimum(return_dates, np.datetime64("2024-09-30"))
    returns = pd.DataFrame({"return_id": [f"RET-{i:07d}" for i in range(1, len(returned) + 1)], "order_id": returned["order_id"].to_numpy(), "return_date": return_dates, "return_reason": rng.choice(["Wrong Size", "Damaged", "Changed Mind", "Not as Expected"], len(returned), p=[.31, .19, .28, .22]), "refund_amount": returned["net_amount"].to_numpy(), "reverse_logistics_cost": rng.uniform(280, 520, len(returned)).round(2)})
    returns.to_csv(os.path.join(output_dir, "returns.csv"), index=False)
    with open(os.path.join(output_dir, ".novamart_demo_version"), "w", encoding="utf-8") as handle: handle.write(DEMO_VERSION)
