...
def build_driver_tree(kpi: KPISummary, analytics_data: Dict[str, Any] | None = None) -> DriverNode:
    ...
    evidence_map = {
        "COGS": "ev_cost_inflation", "Delivery Costs": "ev_shipping_surge", "Marketing Spend": "ev_marketing_cac",
        "Operating Expenses": "ev_cost_inflation", "Returns & Reverse Logistics": "ev_returns_breakdown",
    }
    cost_abs = sum(abs(item[3]) for item in cost_items) or 1
    cost_children = [_node(
        f"drv_cost_{name.lower().replace(' ', '_').replace('&', 'and')}", name, name,
        prior, current, impact, cost_abs, evidence_id=evidence_map.get(name)
